...
# reusable theme
theme_mygraphs <- function(base_size = 20) {
...
labled_plots <- labled_plots %>%
  ...
  mutate (plant_type = (
    ifelse(plant_type == "eergreen shrub", "evergreen shrub", plant_type)
  )) %>%
  left_join(above_sample_dates, by = "plot")
...
spectra_vect_points <- vect (labled_plots, geom = c("lon", "lat"), crs =
                               WGSCRS)
...
# lumping dates for mapping
# converting dates to doy
spectra_plots_date_sampled <- spectra_precip_zones %>%
  mutate(
    date_converted = mdy(date),
    # Converts "7/10/2022" to proper Date
    day_of_year = yday(date_converted)   # Gets day of year
  )
...
spectra_high_n_vect <- vect (spectra_high_n_points,
                             geom = c("lon", "lat"),
                             crs = WGSCRS)
spectra_med_n_vect <- vect (spectra_med_n_points,
                            geom = c("lon", "lat"),
                            crs = WGSCRS)
spectra_low_n_vect <- vect (spectra_low_n_points,
                            geom = c("lon", "lat"),
                            crs = WGSCRS)
...
jpeg("graphs/Figure1_precip_map.jpg",
     width = 800,
     height = 520)
...
pal <- colorRampPalette(c("#E9E4D7", "#3A9BBA"))
...
precip_map <- plot (
  ...
  col = pal(20),
  axes = FALSE,
  plg = list(title = "Winter Precip. (mm)")
)
...
precip_map <- points (high_n_proj, cex = 3, col = "#2646a650")
precip_map <- points (med_n_proj, cex = 2, col = "#2646a650")
precip_map <- points (low_n_proj, cex = 1, col = "#2646a650")

#adding spectra points
precip_map <- points (spectra_high_n_proj,
                      cex = 3,
                      col = "#a6262650")
precip_map <- points (spectra_med_n_proj, cex = 2, col = "#a6262650")
precip_map <- points (spectra_low_n_proj, cex = 1, col = "#a6262650")

# snowfence sites
precip_map <- points (
  snowfence_points_proj,
  cex = 3,
  bg = c("#661100", "#E283AA", "#6699CC"),
  col = "#000000",
  pch = 21
)

sites <- st_coordinates(st_as_sf(snowfence_points_proj))

precip_map <- text(
  sites,
  labels = snowfence_points_proj$site,
  cex = 1.5,
  pos = 4,
  offset = 1.0,
  colour = "black"
...
legend(
  ...
  y = -1900000,
  legend = c(
    "Observation Data High Leaf N (>3%)",
    "Observation Data Med Leaf N (2-3%)",
    "Observation Data Low Leaf N (<2%)",
    "Spectra Data High Leaf N (>3%)",
    "Spectra Data Med Leaf N (2-3%)",
    "Spectra Data Low Leaf N (<2%)",
    "Snow fence sites"
  ),
  pch = c(21, 21, 21, 21, 21, 21, 21),
  col = c(
    "#2646a6",
    "#2646a6",
    "#2646a6",
...
precip_pft_mod <- brm (n ~ mean_precip * functional_group + (1 |
...
precip_pft_mod <- readRDS("models/obs_precip_mod_1km_2.RDS")

precip_pft_mod <- readRDS("models/obs_precip_mod_1km.RDS")

precip_pft_mod <- readRDS ("models/obs_precip_mod_1km_3.RDS")

print(summary(precip_pft_mod), digits = 4)

# Spectra precip model
spectra_precip_mod <- brm (n ~ mean_precip * plant_type + (1 |
                                                             Genus), data = spectra_precip_zones)
...
obs_precip_mod_data <- expand_grid(
  mean_precip = seq(0, 90, by = 5),
  n = seq(0, 5, by = 0.5),
...
(
  obs_precip_mod_fit <- ggplot() +
    geom_point(
      ...
      aes(
        x = mean_precip,
        y = leafn,
        color = ordered (functional_group),
        fill = ordered (functional_group)
      )
    ) +   # raw data
    stat_lineribbon(
      data = obs_precip_mod_pred,
      aes(
        y = .epred,
        x = mean_precip,
        color = ordered (functional_group),
    ...
    scale_fill_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
    scale_color_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
  ...
  annotate(
    ...
    y = Inf,
    hjust = -0.5,
    # Aligns text to the left of the x-position
    vjust = 1,
    # Aligns text to the top of the y-position
    label = "A",
    size = 5,
    color = "black"
  )
...
# dummy datasheet
spectra_precip_mod_data <- expand_grid(
  mean_precip = seq(4, 36, by = 1),
  n = seq(0, 5, by = 0.5),
...
(
  spectra_precip_mod_fit <- ggplot () +
    geom_point(data = spectra_precip_zones, aes(
      x = mean_precip,
      y = n,
      color = ordered (plant_type),
      fill = ordered (plant_type)
    )) +   # raw data
    stat_lineribbon(
      data = spectra_precip_mod_pred,
      aes(
        y = .epred,
        x = mean_precip,
        color = ordered (plant_type),
...
y.grob <- textGrob("Leaf Nitrogen Concentration (%)",
                   gp = gpar(fontsize = 10),
                   rot = 90)
x.grob <- textGrob("Mean Precipitation (mm)", gp = gpar(fontsize = 10))

(
  precip_combined_plot <- grid.arrange(
    obs_precip_plot_simple,
    spectra_precip_mod_fit,
    ncol = 1,
    left = y.grob,
    bottom = x.grob
  )
...
ggsave (
  precip_combined_plot,
  filename = "graphs/Figure5_precip_combined_plot.png",
  width = 8,
  height = 6
)
...
try_snowmelt <- try_snowmelt_raw %>%
  ...
  rename (species = species.x, data_source = data_source.x)
...
madi_snowmelt <- n_precip_zones %>%
  ...
  mutate (snowmelt_date = case_when (grepl (60.98, lat) ~ 167, grepl (69.5971, lat) ~ 158)) %>%
...
# obs model
obs_snowmelt_mod <- brm (leafn ~ snowmelt_date * functional_group + (1 |
                                                                       genus), data = obs_snowmelt)
...
# spectra model
# model
spectra_snowmelt_mod <- brm (n ~ snowmelt_date * plant_type + (1 |
                                                                 Genus), data = spectra_snowmelt_n)
...
# obs model plot
# dummy data frame
obs_snowmelt_mod_data <- expand_grid(
  snowmelt_date = seq(100, 215, by = 5),
  n = seq(0, 5, by = 0.5),
...
spectra_snowmelt_mod_data <- expand_grid(
  snowmelt_date = seq(75, 190, by = 1),
  n = seq(0, 5, by = 0.5),
...
(
  spectra_snowmelt_mod_fit <- ggplot() +
    ...
    stat_lineribbon(
      data = spectra_snowmelt_mod_pred,
      aes(
        y = .epred,
        x = snowmelt_date,
        color = ordered (plant_type),
...
y.grob <- textGrob("Leaf Nitrogen Concentration (%)",
                   gp = gpar(fontsize = 12),
                   rot = 90)
x.grob <- textGrob("Snowmelt Date", gp = gpar(fontsize = 12))
...
ggsave (
  snowmelt_combined_plot,
  filename = "graphs/Figure6_snowmelt_combined_plot.png",
  width = 8,
  height = 6
)
...
# plot!
(
  obs_whittaker <- ggplot (
    data = obs_species_avg,
    aes (x = avg_temp, y = avg_precipitation, col = functional_group)
  ) +
    scale_color_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
    ...
    #scale_x_continuous(expand = c(0, 0), limits = c(-13,1)) +
    theme_classic() +
    theme(
      legend.title = element_blank(),
      legend.position = "none",
...
x.grob <- "Average Annual Temperature (°C)"

# save plot
ggsave (obs_whittaker, filename = "graphs/obs_whittaker.png")

(
  whittaker_spectra <- ggplot (data = spectra_species_avg, aes (
    x = avg_temp, y = avg_precipitation, col = plant_type
  )) +
    scale_color_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
    xlab ("") +
    ylab ("") +
...
whittaker_legend <- get_legend (
  whit_legend_plot <- ggplot (data = spectra_species_avg, aes (
    x = avg_temp, y = avg_precipitation, col = plant_type
  )) +
    geom_point(aes(size = avg_nitrogen))  +
...
whittaker_combined <- grid.arrange (
  obs_whittaker,
  whittaker_spectra,
  right = whittaker_legend,
  ncol = 2,
  bottom = x.grob
)
...
ggsave(
  whittaker_combined,
  filename = "graphs/FigureS2_whittaker_spectra.png",
  width = 8,
  height = 3.5
)
...
legend <- get_legend (
  legend_plot <- ggplot (
    data = obs_species_avg,
    aes (x = avg_precipitation, y = avg_nitrogen, col = functional_group)
  ) +
    geom_point (size = 3) +
    # stat_ellipse(geom = "polygon", aes(x=avg_precipitation, y=avg_nitrogen,
    #                                   fill = functional_group,
...
(
  precip_species_avg_n_obs_plot <- ggplot (
    data = obs_species_avg,
    aes (x = avg_precipitation, y = avg_nitrogen, col = functional_group)
  ) +
    geom_point (size = 3) +
...
(
  precip_species_avg_n_spectra_plot <- ggplot (
    data = spectra_species_avg,
    aes (x = avg_precipitation, y = avg_nitrogen, col = plant_type)
  ) +
    geom_point (size = 3) +
    scale_color_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
    ylab ("") +
...
ggsave (precip_species_avg_n_spectra_plot, filename = "graphs/precip_species_avg_n_spectra_plot.png")
...
obs_species_snowmelt_avg <- obs_snowmelt %>%
  filter(species %in% obs_common_species) %>%
  group_by (functional_group, genus, species) %>%
  summarise(avg_nitrogen = mean(leafn),
...
# plot
(
  snowmelt_species_avg_n_obs_plot <- ggplot (
    data = obs_species_snowmelt_avg,
    aes (x = avg_snowmelt, y = avg_nitrogen, col = functional_group)
  ) +
    geom_point (size = 3) +
    # stat_ellipse(geom = "polygon", aes(x=avg_precipitation, y=avg_nitrogen,
    #                                   fill = functional_group,
    ...
    scale_x_continuous(
      limits = c(80, 230),
      breaks = seq(80, 230, by = 40),
      labels = average_dates
    ) +
...
(
  snowmelt_species_avg_n_spectra_plot <- ggplot (data = spectra_snowmelt_avg, aes (
    x = avg_snowmelt, y = avg_nitrogen, col = plant_type
  )) +
    geom_point (size = 3) +
    scale_color_manual(values = c("#85c94950", "#4E9EE450", "#e8ae2550")) +
    ylab ("") +
...
### combine supplement plots ###

(
  supplement_plots_combined <- grid.arrange (
    precip_species_avg_n_obs_plot,
    precip_species_avg_n_spectra_plot,
    snowmelt_species_avg_n_obs_plot,
    snowmelt_species_avg_n_spectra_plot,
    right = legend,
    left = y.grob,
    ncol = 2
  )
...
(whittaker_combined <- grid.arrange (obs_whittaker, whittaker_spectra, ncol = 2))

ggsave (
  supplement_plots_combined,
  filename = "graphs/FigureS3_supplement_plots_combined.png",
  width = 8,
  height = 6
)
