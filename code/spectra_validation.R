...
(
  validation_plot <- ggplot (comparison, aes(x = spectral_n, y = chemistry_n)) +
    ...
    xlab (
      "\nEstimates of Leaf Nitrogen from Leaf Hyperspectral Reflectance (%)"
    ) +
    ylab ("Direct Measuremnts of Leaf Nitrogen Concentration (%)\n") +
...
ggsave (
  validation_plot,
  filename = "graphs/FigureS1_validation_plot.png",
  width = 6,
  height = 5
)
...
sqrt(mean((
  comparison$chemistry_n - comparison$spectral_n
)^2))
# 0.6679244
